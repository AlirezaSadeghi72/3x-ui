...
iplimit_log_path="${log_folder}/3x1ipl.log"
iplimit_banned_log_path="${log_folder}/3x1ipl-banned.log"
...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/AlirezaSadeghi72/3x1-ui/main_2.4.11/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
update_menu() {
    ...
    wget --no-check-certificate -O /usr/bin/x1-ui https://raw.githubusercontent.com/AlirezaSadeghi72/3x1-ui/main_2.4.11/x1-ui.sh
    chmod +x /usr/local/x1-ui/x1-ui.sh
    chmod +x /usr/bin/x1-ui
...
legacy_version() {
    ...
    install_command="bash <(curl -Ls "https://raw.githubusercontent.com/AlirezaSadeghi72/3x1-ui/main_2.4.11/install.sh") v$tag_version"
...
uninstall() {
    ...
    systemctl stop x1-ui
    systemctl disable x1-ui
    rm /etc/systemd/system/x1-ui.service -f
    systemctl daemon-reload
    systemctl reset-failed
    rm /etc/x1-ui/ -rf
    rm /usr/local/x1-ui/ -rf

    echo ""
    echo -e "Uninstalled Successfully.\n"
    echo "If you need to install this panel again, you can use below command:"
    echo -e "${green}bash <(curl -Ls https://raw.githubusercontent.com/AlirezaSadeghi72/3x1-ui/master/install.sh)${plain}"
    echo ""
...
reset_user() {
    ...
    /usr/local/x1-ui/x1-ui setting -username ${config_account} -password ${config_password} >/dev/null 2>&1
    /usr/local/x1-ui/x1-ui setting -remove_secret >/dev/null 2>&1
    echo -e "Panel login username has been reset to: ${green} ${config_account} ${plain}"
    echo -e "Panel login password has been reset to: ${green} ${config_password} ${plain}"
    echo -e "${yellow} Panel login secret token disabled ${plain}"
    echo -e "${green} Please use the new login username and password to access the x1-ui panel. Also remember them! ${plain}"
    confirm_restart
}
...
reset_webbasepath() {
    ...
    /usr/local/x1-ui/x1-ui setting -webBasePath "${config_webBasePath}" >/dev/null 2>&1
...
reset_config() {
    ...
    /usr/local/x1-ui/x1-ui setting -reset
    echo -e "All panel settings have been reset to default."
    restart
}

check_config() {
    local info=$(/usr/local/x1-ui/x1-ui setting -show true)
    if [[ $? != 0 ]]; then
    ...
    local existing_cert=$(/usr/local/x1-ui/x1-ui setting -getCert true | grep -Eo 'cert: .+' | awk '{print $2}')
...
set_port() {
    ...
    if [[ -z "${port}" ]]; then
        ...
        /usr/local/x1-ui/x1-ui setting -port ${port}
...
start() {
    ...
    if [[ $? == 0 ]]; then
        ...
        systemctl start x1-ui
        sleep 2
        check_status
        if [[ $? == 0 ]]; then
            LOGI "x1-ui Started Successfully"
        else
...
stop() {
    ...
    if [[ $? == 1 ]]; then
        ...
        check_status
        if [[ $? == 1 ]]; then
            LOGI "x1-ui and xray stopped successfully"
        else
...
restart() {
    ...
    check_status
    if [[ $? == 0 ]]; then
        LOGI "x1-ui and xray Restarted successfully"
    else
...
enable() {
    ...
    if [[ $? == 0 ]]; then
        LOGI "x1-ui Set to boot automatically on startup successfully"
    else
        LOGE "x1-ui Failed to set Autostart"
    fi
...
disable() {
    systemctl disable x1-ui
    if [[ $? == 0 ]]; then
        LOGI "x1-ui Autostart Cancelled successfully"
    else
        LOGE "x1-ui Failed to cancel autostart"
    fi
...
show_log() {
    ...
    0)
        ...
        ;;
    1)
        journalctl -u x1-ui -e --no-pager -f -p debug
        if [[ $# == 0 ]]; then
        before_show_menu
        fi
        ;;
...
show_banlog() {
    ...
    if [[ -f "$system_log" ]]; then
        ...
        grep "3x1-ipl" "$system_log" | grep -E "Ban|Unban" | tail -n 10 || echo -e "${yellow}No recent system ban activities found${plain}"
        echo ""
    fi

    if [[ -f "${iplimit_banned_log_path}" ]]; then
        echo -e "${green}3x1-ipl ban log entries:${plain}"
    ...
    fail2ban-client status 3x1-ipl || echo -e "${yellow}Unable to get jail status${plain}"
}
...
update_shell() {
    wget -O /usr/bin/x1-ui -N --no-check-certificate https://github.com/AlirezaSadeghi72/3x1-ui/raw/main/x1-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
...
check_status() {
    if [[ ! -f /etc/systemd/system/x1-ui.service ]]; then
        return 2
    fi
    temp=$(systemctl status x1-ui | grep Active | awk '{print $3}' | cut -d "(" -f2 | cut -d ")" -f1)
...
check_enabled() {
    temp=$(systemctl is-enabled x1-ui)
        ...
        if [ -z "$domains" ]; then
            ...
            if echo "$domains" | grep -qw "$domain"; then
                ...
                if [[ -f "${webCertFile}" && -f "${webKeyFile}" ]]; then
                    /usr/local/x1-ui/x1-ui cert -webCert "$webCertFile" -webCertKey "$webKeyFile"
...
ssl_cert_issue() {
    local existing_webBasePath=$(/usr/local/x1-ui/x1-ui setting -show true | grep -Eo 'webBasePath: .+' | awk '{print $2}')
    local existing_port=$(/usr/local/x1-ui/x1-ui setting -show true | grep -Eo 'port: .+' | awk '{print $2}')
...
create_iplimit_jails() {
    ...
    cat << EOF > /etc/fail2ban/jail.d/3x1-ipl.conf
[3x1-ipl]
enabled=true
backend=auto
filter=3x1-ipl
...
EOF
    ...
    cat << EOF > /etc/fail2ban/filter.d/3x1-ipl.conf
[Definition]
...
EOF
    ...
    cat << EOF > /etc/fail2ban/action.d/3x1-ipl.conf
[INCLUDES]
...
iplimit_remove_conflicts() {
    ...
    for file in "${jail_files[@]}"; do
        # Check for [3x1-ipl] config in jail file then remove it
        if test -f "${file}" && grep -qw '3x1-ipl' ${file}; then
            sed -i "/\[3x1-ipl\]/,/^$/d" ${file}
            echo -e "${yellow}Removing conflicts of [3x1-ipl] in jail (${file})!${plain}\n"
        fi
    done
}
...
iplimit_main() {
    ...
    3)
        ...
        if [[ $? == 0 ]]; then
            fail2ban-client reload --restart --unban 3x1-ipl
    ...
    5)
        ...
        if [[ $ban_ip =~ $ipv4_regex || $ban_ip =~ $ipv6_regex ]]; then
            fail2ban-client set 3x1-ipl banip "$ban_ip"
    ...
    6)
        ...
        if [[ $unban_ip =~ $ipv4_regex || $unban_ip =~ $ipv6_regex ]]; then
            fail2ban-client set 3x1-ipl unbanip "$unban_ip"
...
remove_iplimit() {
    ...
    case "$num" in
    1)
        rm -f /etc/fail2ban/filter.d/3x1-ipl.conf
        rm -f /etc/fail2ban/action.d/3x1-ipl.conf
        rm -f /etc/fail2ban/jail.d/3x1-ipl.conf
        systemctl restart fail2ban
...
SSH_port_forwarding() {
    ...
    local existing_webBasePath=$(/usr/local/x1-ui/x1-ui setting -show true | grep -Eo 'webBasePath: .+' | awk '{print $2}')
    local existing_port=$(/usr/local/x1-ui/x1-ui setting -show true | grep -Eo 'port: .+' | awk '{print $2}')
    local existing_listenIP=$(/usr/local/x1-ui/x1-ui setting -getListen true | grep -Eo 'listenIP: .+' | awk '{print $2}')
    local existing_cert=$(/usr/local/x1-ui/x1-ui setting -getCert true | grep -Eo 'cert: .+' | awk '{print $2}')
    local existing_key=$(/usr/local/x1-ui/x1-ui setting -getCert true | grep -Eo 'key: .+' | awk '{print $2}')
    ...
    1)
        if [[ -z "$existing_listenIP" || "$existing_listenIP" == "0.0.0.0" ]]; then
            ...
            /usr/local/x1-ui/x1-ui setting -listenIP "${config_listenIP}" >/dev/null 2>&1
        ...
        ;;
    2)
        /usr/local/x1-ui/x1-ui setting -listenIP 0.0.0.0 >/dev/null 2>&1
...
show_usage() {
    echo -e "┌───────────────────────────────────────────────────────┐
│  ${blue}x1-ui control menu usages (subcommands):${plain}              │
│                                                       │
│  ${blue}x1-ui${plain}              - Admin Management Script          │
│  ${blue}x1-ui start${plain}        - Start                            │
│  ${blue}x1-ui stop${plain}         - Stop                             │
│  ${blue}x1-ui restart${plain}      - Restart                          │
│  ${blue}x1-ui status${plain}       - Current Status                   │
│  ${blue}x1-ui settings${plain}     - Current Settings                 │
│  ${blue}x1-ui enable${plain}       - Enable Autostart on OS Startup   │
│  ${blue}x1-ui disable${plain}      - Disable Autostart on OS Startup  │
│  ${blue}x1-ui log${plain}          - Check logs                       │
│  ${blue}x1-ui banlog${plain}       - Check Fail2ban ban logs          │
│  ${blue}x1-ui update${plain}       - Update                           │
│  ${blue}x1-ui legacy${plain}       - legacy version                   │
│  ${blue}x1-ui install${plain}      - Install                          │
│  ${blue}x1-ui uninstall${plain}    - Uninstall                        │
└───────────────────────────────────────────────────────┘"
}
...
show_menu() {
    echo -e "
╔────────────────────────────────────────────────╗
│   ${green}3x1-ui Panel Management Script${plain}                │
